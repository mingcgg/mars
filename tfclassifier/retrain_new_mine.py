...
images.
#　在顶（最外）层上每张图片接收2048大小的向量，用SOFTMAX来表示，假设SOFTMAX层有N个标签，对应地就要学习 N +　２０４８　*　Ｎ　个偏置和权值参数
...
def create_inception_graph():
  ...
  Returns:
    ...
    manipulating.
    Ｇraph:
    bottleneck_tensor: shape=(1, 2048) pool_3/_reshape
  ...
  with tf.Session() as sess:
    ...
    with gfile.FastGFile(model_filename, 'rb') as f:
      ...
      #此函数提供了一种导入序列化的 TensorFlow GraphDef协议缓冲区的方法，
      # 并将 GraphDefas 中的各个对象提取为 tf.Tensor 和 tf.Operation 对象。一旦提取，这些对象将被放置到当前的默认 Graph 中
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
  ...
  Args:
    ...
    image_data: String of raw JPEG data. bytes[]
    image_data_tensor: Input data layer in the graph. 图片输入模块节点 DecodeJpeg/contents
    bottleneck_tensor: Layer before the final softmax. softmax之前的最后一层

  Returns:
    Numpy array of bottleneck values. len=2048,将类型判断为２０４８种 shape (1, 2048)
  """
  bottleneck_values = sess.run(
      bottleneck_tensor,
      {image_data_tensor: image_data})
  bottleneck_values = np.squeeze(bottleneck_values) # 降维操作wm shape中为1的维度去掉
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
  ...
  bottleneck_values = [float(x) for x in bottleneck_string.split(',')]#len 2048
  return bottleneck_values
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope('input'):
    bottleneck_input = tf.placeholder_with_default( # key code core
  ...
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      # 日期20181116 (N, 2048) * (2048, classcount)  layer_weights classcount * 2048就是训练参数的个数，与上一层（模型的输出）连接
      # 个人理解： 上一层（根据模型计算出来 的百分比，权值，就已经确定了是哪一类，有一个高百分比的位， 与(2048, classcount) 相连接，收缩为我们需要的种类
      # 就是说我已经认识了上千种东西，再让我认出这其中的几十种，稍微训练一下就行了，so easy
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--image_dir',
      type=str,
      default= 'D:/research/tensorflow/ai_challenger_zsl2018/DataSet/Animals', #'./images',
