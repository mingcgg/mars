...
def add_distortions(flip_left_right=False, random_crop=100, random_scale=100, random_brightness=100):
    jpeg_data = tf.placeholder(tf.string, name='DistortJPGInput')
    decoded_image = tf.image.decode_jpeg(jpeg_data, channels=MODEL_INPUT_DEPTH)
    decoded_image_as_float = tf.cast(decoded_image, dtype=tf.float32)
    decoded_image_4d = tf.expand_dims(decoded_image_as_float, 0)
...
def get_distortions(jpeg_data, flip_left_right=False, random_crop=100, random_scale=100, random_brightness=100):
    decoded_image = tf.image.decode_jpeg(jpeg_data, channels=MODEL_INPUT_DEPTH)
    decoded_image_as_float = tf.cast(decoded_image, dtype=tf.float32)
    decoded_image_4d = tf.expand_dims(decoded_image_as_float, 0)
    margin_scale = 1.0 + (random_crop / 100.0)
...
#image_placeholder, distort_result = test()
#print(image_placeholder, distort_result) add_input_distortions(True, 50, 50, 50)